...
---
> 메모리의 단편화<br>
- 메모리 구성과 관계업ㄱ이 모든 컴퓨터 시스템에서 발생
---
> 주소 바인딩<br>
- 논리적 주소를 물리적 주소로 변환하는 과정
---
> 동적 분할기법<br>
- 각 프로세스의 크기에 따라 메모리를 분할하는 방식 -> 고정 분할기법의 문제점을 해결하기 위함
---
> 메모리 관리 기법<br>
```
- 반입 정책 : 메인 메모리를 적재할 다음 프로세스의 반입시기를 결정하는 방법(요구 반입과 예상 반입이있음)
- 배치 정책 : 반입한 프로세스를 어느 위치에 저장할 것인지 결정하는 방법
- 대치 정책 : 재배치 기법으로 메인 메모리에 있는 어떤 프로세스를 제거할 것인가를 결정
```
---
> 메모리 관리<br>
- 다중 프로그래밍 시스템에서
...
- 실행에 반드시 필요한 명령어와 데이터를 저장
---
> 고정 분할 다중프로그래밍에서의 단편화<br>
- 작업 스케줄러와 분할의 크기에 의해 좌우된다.
---
> 이지 테이블<br>
- 전용 레지스터를 사용해 구현 -> 페이징 주소 변환을 매우 효율적으로 하기위해 초고속 논리회로로 설계
---
> 가변 분할 다중 프로그래밍<br>
- 고정된 경계를 없에고 각 프로세스가 필요한 만큼만 메모리를 할당하는 방법
---
> 메모리 재배치 기법<br>
```
- 최초적합 : 프로세스는 사용가능 공간 리스트에서 충분히 큰 첫번째 공백 분할 공간에 할당, 검색은 빠르나 공간활용률이 떨어짐
- 최상적합 : 프로세스가 들어갈 수 있는 충분히 큰 사용가능공간 중에서 가장 작은 크기의 사용 공간에 작업을 할당, 지속적인 정렬과정이 필요해 비효율적, 이용률은 향상되나 할당에 많은 시간이 소요
- 최악적합 : 작업을 가장 큰 사용공간에 할당, 크기 순서로 정렬되어있지 않을 시 전 리스트를 검색해야됨, 최상 적합보다 메모리 활용면에서 더 유용함
```
---
> 페이징과 세그먼트<br>
```
- 페이징 : 동일한 크기, 내부 단편화현상
- 세그먼트 : 가변 크기, 외부 단편화 현상
```
---
> 가상 메모리<br>
```
- 공간이 부족해도 부분적재가 가능하여 많은 작업을 실행할 수 있어 프로세서의 이용률과 처리률이 향상됨
- 계층 구조에서 메인 메모리와 연관되어 동작
```
---
> 페이지 테이블 항목(PTE)의 제어비트<br>
```
...
```
<장점>
- 프로그래밍 작업이 쉬우며, 중첩을 작성할 필요가 없음
- 부분 적재가 가능하여 많은 작업을 실행 가능 -> 프로세서의 이용률과 처리율 향상
<단점>
...
---
> 사상(Mapping)<br>
- 가상 주소를 실제 주소로 변환하는 과정 -> F : V = R
---
> 블록 사상<br>
- 사상 정보의 양(테이블)을 줄이기 위해 주소를 블록단위로 처리
---
> 요구 페이징 기법<br>
```
- 스왑 기법을 사용하는 페이징 시스템과 비슷
- 프로그램을 실행하기 위해 프로그램의 일부만을 메인 메모리에 적재
- 프로세스는 디스크에 저장되었다가 수행될 때, 직접 적재하지 않고 지연 교환기를 사용해 메인 메모리에 적재함
```
---
> 스레싱 발생원인<br>
- 스레싱이란 : 페이지 교환이 계속 일어나는 현상
```
...
- 페이지 부재가 많이 일어날수록 프로세스가 페이징 처리 장치를 기다리는 시간이 깅어지므로 프로세스의 효율성이 떨어짐
- 페이지 부재로 프로세서의 이용률 감소시 스레싱이 발생하여 시스템의 처리율은 낮아지고 페이지 부재는 늘어나 유효 메모리 액세스 시간이 증가, 페이지 교체시간이 낭비됨
```
---
> 지역성(국부성)<br>
...
```
<종류>
- 시간 지역성 : 참조된 기억장소는 가까운 미래에도 계 속 참조될 가능성이 높음을 의미 
- 공간 지역성 : 프로세스가 어떤 기억장소를 한 번 참조하면, 이후에 참조한 기억장치 근처에 있는 기억장소를 참조할 가능성이 높음을 의미
```
