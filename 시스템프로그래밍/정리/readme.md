...
---
> 타이머기반 동기화<br>
- 정해진 시간이 지나면 자동으로 Signaled 상태가 된다
```
- 수동 리셋 타이머 : 알람 시계와 같은 형태
- 주기적 타이머 : 일정 시간 간격으로 알람이 울리는 타이머 기반의 동기화 방법
```
---
> 쓰레드의 재활용<br>
- 성능을 향상시킬 수 있다.
---
> 타이머 오브젝트 생성과 관련된 함수<br>
```
- IpTimeAttributes : 보안 속성을 지정
- bNanualReset : 타이머 오브젝트를 수동으로 리셋 모드로 생성할 것인지 자동으로 생성할 것인지
- IpTimeName : 타이머 오브젝트에 이름을 붙여줄 경우 사용되는 전달 인자
...
```
---
> 쓰레드에게 일을 시키기 위한 함수<br>
- 반환형과 매개변수 타입 모두 void형으로 선언
---
> 쓰레드에 할당된 일이 없는 경우 <br>
- waitFor ~ : 관련함수 도출을 통해 block한다
---
> 쓰레드 풀<br>
```
- WorkList는 work를 등록하는 저장소이다.
- work가 등록될 위치를 가리키는 인자가 필요하다 -> idxofLastAddedWork
- 처리해야할 위치를 가리키는 인자가 필요하다 -> indxOfCurrentWork
- 저장된 쓰레드의 개수 정보 -> threadidx
```
---
> 쓰레드 풀을 구현하기 위해 필요한 멤버<br>
```
- Work에 관련된 멤버
- 쓰레드에 관련된 멤버
- 일에 관련된 멤버
...
4. WaitForSingleObject 함수반환 : Running 상태로 변경
5. Work를 얻음 : GetWorkFromPool
```
---
> 쓰레드 풀의 함수 관계<br>
```
- WORK GetWorkFromPool(void) : 쓰레드 풀에서 Work를 가져올 때 호출하는 함수
- DWORD MakeThreadToPool(Work work) : 새로운 Work를 등록할 때 호출하는 함수
- DWORD MakeThreadToPool(DWORD numOfThread) : 쓰레드 풀이 생성된 이후 풀에 쓰레드를 생성하는 함수 / 인자로 전달되는 수 만큼 쓰레드가 생성
- Void WorkerThreadFunction(LPVOID pParam) : 쓰레드가 생성되자마자 호출하는 쓰레드의 main함수 -> work 할당 및 처리 방법 그리고 Work이 없을 경우 쓰레드 상태 확인 가능
```
---
> 메모리 속도별 순서<br>
```
- 레지스터 > L1캐시(CPU 내부) > L2캐시(CPU 외부) > RAM > ROM
```
---
> 캐쉬의 데이터의 이동<br>
- 블록단위로 함
---
> L1, L2 캐시<br>
- 메인 메모리를 제외하고 연산에 필요한 데이터가 90%이상 존재하는 메모리
---
> 컴퓨터 프로그램의 일반적인 특성<br>
```
- Temporal Locality : 프로그램 실행시 한번 접근이 이워진 주소의 메모리 영역은 자주 접근하게 된다는 프로그램 특성을 표현할 때 사용
- Spatial Locality : 프로그램 실행 시 접근하는 메모리 영역은 이미 접근이 이루어지 ㄴ영역의 근처일 확률이 높다는 프로그램 성젹을 표현 / 접근한 메모리 근처메모리를 접근할 확률이 높다는 성격
- Cache friendly Code : 캐쉬의 도움을 많이 받는 프로그램
```
---
